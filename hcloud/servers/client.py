...
from hcloud.servers.domain import (CreateServerResponse, EnableRescueResponse,
                                   IPv4Address, IPv6Network, PrivateNet,
                                   PublicNetwork, RequestConsoleResponse,
                                   ResetPasswordResponse, Server)
from hcloud.volumes.client import BoundVolume

if TYPE_CHECKING:
    from typing import Any, Dict, List, Optional, Union

    from hcloud import actions, servers
    ...
    def __init__(self, client, data, complete=True):
        ...
        if public_net:
            ...
            data["public_net"] = PublicNetwork(
                ipv4=ipv4_address, ipv6=ipv6_network, floating_ips=floating_ips
            )
...
class ServersClient(ClientEntityBase, GetEntityByNameMixin):
    ...
    def change_type(self, server, server_type, upgrade_disk):
        # type: (servers.domain.Server, BoundServerType, bool) -> actions.domain.Action
        """Changes the type (Cores, RAM and disk sizes) of a server.
    ...
    def enable_rescue(self, server, type=None, ssh_keys=None):
        ...
        :return: :class:`EnableRescueResponse <hcloud.servers.domain.EnableRescueResponse>`
        """
        data = {"type": type}  # type: Dict[str, Any]
        if ssh_keys is not None:
    ...
    def rebuild(self, server, image):
        # type: (servers.domain.Server, Image) -> actions.domain.Action
        """Rebuilds a server overwriting its disk with the content of an image, thereby destroying all data on the target server.
        ...
        :return:  :class:`BoundAction <hcloud.actions.client.BoundAction>`
        """
        data = {"image": image.id_or_name}  # type: Dict[str, Any]
        response = self._client.request(
    ...
    def attach_iso(self, server, iso):
        # type: (servers.domain.Server, Iso) -> actions.domain.Action
        """Attaches an ISO to a server.
        ...
        :return:  :class:`BoundAction <hcloud.actions.client.BoundAction>`
        """
        data = {"iso": iso.id_or_name}  # type: Dict[str, Any]
        response = self._client.request(
    ...
    def change_dns_ptr(self, server, ip, dns_ptr):
        # type: (servers.domain.Server, str, str) -> actions.domain.Action
        """Changes the hostname that will appear when getting the hostname belonging to the primary IPs (ipv4 and ipv6) of this server.
        ...
        :return:  :class:`BoundAction <hcloud.actions.client.BoundAction>`
        """
        data = {"ip": ip, "dns_ptr": dns_ptr}  # type: Dict[str, Any]
        response = self._client.request(
    ...
    def change_protection(self, server, delete=None, rebuild=None):
        # type: (servers.domain.Server, Optional[bool], Optional[bool]) -> actions.domain.Action
        """Changes the protection configuration of the server.
    ...
    def change_alias_ips(self, server, network, alias_ips):
        ...
        :return: :class:`BoundAction <hcloud.actions.client.BoundAction>`
        """
        data = {"network": network.id, "alias_ips": alias_ips}  # type: Dict[str, Any]
        response = self._client.request(
